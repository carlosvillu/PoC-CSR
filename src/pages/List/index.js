...
const qs = ({id, tag}) =>
  id !== undefined ? `?id=${id}` : tag !== undefined ? `?tag=${tag}` : ''
const fetch = ({component, query}) =>
  axios.get(`${END_POINT}${qs(query)}`).then(resp => {
...
export default compose(
  ...
  withProps(ownerProps => ({
    canonical: `https://poccsr.ovh/people${qs(ownerProps.location.query)}`
  }))
)(List)
