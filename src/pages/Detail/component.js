  ...
  return (
    <div className="Detail">
      <Helmet>
        ...
        <meta
          name="description"
          content={`${name} es una persona inquieta. ${bio}`}
        />
      ...
      <h2>BreadCrumb</h2>
      <Link className="Detail-BreadCrumb" to="/">
        Prueba Indexación JS (a home)
      </Link>
      <Link className="Detail-BreadCrumb" to="/people">
        Las 5 personas de la prueba
      </Link>
      <p className="Detail-BreadCrumb">{name}</p>
      <h2>Tags</h2>
      {tags.map(tag => (
        <Link className="Detail-Tag" key={tag} to={`/people?tag=${tag}`}>
          {tag}
        </Link>
      ))}
      <h2>Foto</h2>
